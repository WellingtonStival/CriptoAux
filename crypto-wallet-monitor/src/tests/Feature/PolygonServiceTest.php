...
use App\Services\Blockchain\PolygonService;
...
class PolygonServiceTest extends TestCase
{
    ...
    public function test_converts_wei_to_pol_correctly(): void
    {
        Http::fake([
            '*' => Http::response([
                'jsonrpc' => '2.0',
                'id' => 1,
                'result' => '0x1bc16d674ec80000', // 2 POL em wei
            ]),
        ]);

        $balance = app(PolygonService::class)->getBalance(self::ADDRESS);

        $this->assertSame(2.0, $balance);
        Http::assertSent(fn ($request) => $request->url() === config('blockchain.polygon.rpc_url'));
    }
    ...
    public function test_aborts_with_502_when_the_rpc_returns_an_error(): void
    {
        ...
        try {
            app(PolygonService::class)->getBalance(self::ADDRESS);
    ...
    {
        $service = app(PolygonService::class);

        $this->assertSame('POL', $service->symbol());
    ...
    public function test_discover_tokens_ignores_the_native_pseudo_contract_and_uses_alchemy(): void
    {
        ...
        Http::fake(function ($request) {
            ...
            if ($method === 'alchemy_getTokenBalances') {
                $this->assertStringContainsString('polygon-mainnet.g.alchemy.com', $request->url());
        ...
        $tokens = app(PolygonService::class)->discoverTokens(self::ADDRESS);
